...
## Analysing Live Headlines from 'The Bridge' Website
...
process_headlines_full <- function(df, text_col, lexicon) {
 ...
 unigram_bar <- ggplot(word_freq %>% slice_max(n, n = 15), 
  ...
  trigram_sankey <- suppressMessages(create_sankey(trigrams, 3))
  quadgram_sankey <- suppressMessages(create_sankey(quadgrams, 4))
  ...
  return(list(
    ...
    df_emotions_filtered = df_emotions_filtered
  ))
}
...
emotion_heat_data <- as.matrix(
  bridge_results$df_emotions_filtered %>%
    select(Positive:Trust)
)

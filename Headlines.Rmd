...
process_headlines_full <- function(df, text_col, lexicon) {
  ...
  # 1. Data Cleaning and Tokenization (Unigrams)
  df <- df %>% 
  ...
  # 3. Visualizations for Word Frequencies
  ## 3a. Unigram Word Cloud (displayed on screen)
  wordcloud(words = word_freq$word, 
            freq = word_freq$n, 
            min.freq = 2, 
            max.words = 100, 
  ...
  # 5. Bar Charts for Trigrams and Quadgrams
  ...
  # 6. Sankey Networks for Trigrams and Quadgrams
  # Helper function: create_sankey() splits the n-gram column into consecutive word links.
  create_sankey <- function(ngram_df, n_parts) {
    # n_parts: number of words in the n-gram (e.g., 3 for trigrams, 4 for quadgrams)
    # We'll create links for each consecutive pair.
  ...
  # 7. Network Graph for Pentagrams
  ...
  # 8. Treemap for Hexagrams using treemapify
  ...
  # 9. Emotion Detection in Headlines
  # Use the previously created tidy_df (for unigrams) for emotion matching
  emotions <- tidy_df %>%
  ...
  # For a heatmap, we need to filter out rows with all zeros from df_emotions:
  emotion_heat_data <- as.matrix(df_emotions %>% select(Positive:Trust))
  # Use base heatmap (this will print directly, no object returned)
  ...
  return(list(
    ...
    # Note: Word clouds and the heatmap are directly drawn to the graphics device.
  ))
}
...
# Display plots (unigram bar chart, trigram/quadgram bar charts, emotion bar chart)
...
bridge_results$quadgram_sankey
...
bridge_results$pentagram_network
...
bridge_results$hexagram_treemap
